...
private:
  ...
  std::map<std::string, std::optional<std::string>> parsed_config(const std::filesystem::path& path)
  {
    ...
    const auto lines = str::file_data_to_strings_if(path, is_nor_empty_nor_commented);
...
DMITIGR_CFG_INLINE std::unique_ptr<Flat> Flat::make(const std::filesystem::path& path)
{
