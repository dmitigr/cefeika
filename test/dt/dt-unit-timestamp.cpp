...
int main(int, char* argv[])
{
  ...
  try {
...

    {
      ...
      ASSERT(ts = Timestamp::from_rfc7231(ts_str));
      ASSERT(ts->day_of_week() == Day_of_week::wed);
      ASSERT(ts->day() == 6);
    ...
    {
      auto ts1 = Timestamp::make();
      auto ts2 = Timestamp::make();
      ASSERT(ts1->is_equal(ts2.get()));

      ts2->set_hour(1);
      ASSERT(ts1->is_less(ts2.get()));
    }
